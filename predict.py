...
def predict(indices, y_pred=None, c_pred=None, img_scaler=(1, 1)):
    ...
    indices = indices.to(device).detach().cpu().numpy() if torch.is_tensor(indices) else indices.numpy()
    # indices = indices.detach().cpu().numpy()if torch.is_tensor(indices) else indices.numpy()
    # indices = (
    #     indices.detach().gpu().numpy() if torch.is_tensor(indices) else indices.numpy()
    # )

    # Transform input for heatmap prediction
    if y_pred is not None:
        y_pred = y_pred > 0.5
        y_pred = y_pred.to(device).detach().cpu().numpy() if torch.is_tensor(y_pred) else y_pred
        y_pred = to_img_format(y_pred)  # (N, L, H, W)

    # Transform input for coordinate prediction
    if c_pred is not None:
        c_pred = c_pred.to(device).detach().cpu().numpy() if torch.is_tensor(c_pred) else c_pred
    ...
    #     # y_pred = y_pred.detach().gpu().numpy() if torch.is_tensor(y_pred) else y_pred
    #     y_pred = to_img_format(y_pred)  # (N, L, H, W)

    # # Transform input for coordinate prediction
    # if c_pred is not None:
    #     c_pred = c_pred.detach().cpu().numpy() if torch.is_tensor(c_pred) else c_pred
    #     # c_pred = c_pred.detach().gpu().numpy() if torch.is_tensor(c_pred) else c_pred
...
if __name__ == "__main__":
    # import sys
    # sys.argv = 'predict.py --video_file 2024-04-22-203629.mp4 --tracknet_file ckpts/TrackNet_best.pt --inpaintnet_file ckpts/InpaintNet_best.pt --save_dir prediction --output_video'.split()
            ...
            for step, (i, coor_pred, inpaint_mask) in enumerate(tqdm(data_loader)):
                ...
                with torch.no_grad():
                    coor_inpaint = (
                        ...
                        # inpaintnet(coor_pred.cuda(), inpaint_mask.cuda()).detach().gpu()
                    )
